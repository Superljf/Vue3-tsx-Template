...
const Login = defineComponent({
	...
	setup(props) {
				...
				<div class={styles['site-content__wrapper']}>
					<div class={styles['site-content']}>
						<div class={styles['login-main']}>
							...
							<Form ref={formRef} rules={rules} model={formData} label-col={labelCol} wrapper-col={wrapperCol}>
								<Form.Item name="username">
									<Input size="large" v-model={[formData.username, 'value']} placeholder="请输入用户名"></Input>
								</Form.Item>
								<Form.Item name="password">
									<Input size="large" type="password" v-model={[formData.password, 'value']} placeholder="请输入密码"></Input>
								</Form.Item>
                    ...
                    <Col span={8} class={styles['login-captcha']}>
                      ...
                      />
								...
								<Form.Item wrapperCol={{ span: 18, offset: 3 }}>
									<Button size="large" loading={loading.value} onClick={handleFormSubmit} class={styles['login-btn-submit']} type="primary">
										登录
									</Button>
								</Form.Item>
							</Form>
		...
		return () => <GlobalBg class={styles['site-wrapper-login']} v-slots={slots}></GlobalBg>;
	}
});
